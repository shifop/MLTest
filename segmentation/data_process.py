...
if __name__ == "__main__":
    ...
    for x in cache:
        ...
        ctag=[]
        for words in x['seq']:
            chart.extend([c for c in words])
            if len(words)>=2:
                ctag.append('b')
                ctag.extend(['m' for i in range(len(words)-2)])
                ctag.append('e')
            else:
                ctag.append('s')

        train_data.append({'seq': chart,'tag': ctag})
    ...
    # 生成records
    train_writer = tf.python_io.TFRecordWriter('../data/seg/train.record')
    ...
    dev_writer = tf.python_io.TFRecordWriter('../data/seg/dev.record')
    ...
    # 保存各类map
    with open('../data/seg/w2i.json','w',encoding='utf-8') as f:
        f.write(json.dumps(w2i, ensure_ascii=False))

    with open('../data/seg/i2w.json','w',encoding='utf-8') as f:
        f.write(json.dumps(i2w, ensure_ascii=False))

    with open('../data/seg/t2i.json','w',encoding='utf-8') as f:
