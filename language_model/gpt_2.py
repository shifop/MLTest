...
if __name__=='__main__':
    ...
    with graph.as_default():
        x = tf.placeholder(tf.int32,[64,200])
        past = tf.placeholder(tf.float32, [64,12,2,12,200,64])
        rt = model(hparams,x,past,reuse=tf.AUTO_REUSE)
        tf.nn.nce_loss
        # rt2 = model(hparams, x, past, reuse=tf.AUTO_REUSE)
        for index,t in enumerate(tf.trainable_variables()):
            print('%d:%s'%(index,t))